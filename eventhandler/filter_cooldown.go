...
func CoolDownFilterTimeProvider(provider clock.TimeProvider) CoolDownFilterOption {
...
func NewCoolDownFilter(cooldown int64, logger *zap.SugaredLogger, options ...CoolDownFilterOption) *CoolDownFilter {
	...
	opt := &CoolDownFilterOptions{
		timeProvider: &clock.DefaultTimeProvider{},
	}
...
func (h *CoolDownFilter) Handle(event *extevent.Event) {
	if h.coolDown == 0 {
		h.callNext(event)
		return
	}
